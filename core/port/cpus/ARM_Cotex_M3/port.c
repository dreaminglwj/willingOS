...
__asm void pendSVHandler( void ) {
    ...
    ldr r2, [r3] /* 以r3中的内容为地址，将该地址中的数据传到R2中，取currentTCB地址 */

    stmdb r0!, {r4-r11} /* 保存剩下的寄存器值，R4~R11这些寄存器系统不会主动入栈，需要手动推到当前任务的堆栈 bug202401090001：这里保存寄存器的时候导致内存溢出 */ 
    str r0, [r2] /* 保存当前任务的栈顶指针：将r0寄存器的值（PSP）保存到currentTCB指向的任务控制块中，即将PSP的值保存为任务的堆栈指针，因为tcb中stacktop在最上面 */
